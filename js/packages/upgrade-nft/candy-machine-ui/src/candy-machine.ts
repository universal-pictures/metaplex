...
export const burnToken = async (
  ...
  const instructions = [
    Token.createBurnInstruction(
      TOKEN_PROGRAM_ID,
      new anchor.web3.PublicKey(mintAddress), // Mint Address PubKey
      tokenAddress, // Token Address PubKey
      payer,
      [],
      1,
    )
...
export const getTokenAccount = async (
  ...
  // const toTokenAccount = await getOrCreateAssociatedTokenAccount(candyMachine.program.provider.connection, payer, new anchor.web3.PublicKey(mintAddress), payer.publicKey);
  const tokenAcct = await getAtaForMint(new anchor.web3.PublicKey(mintAddress), payer);
  console.log(tokenAcct);
  return tokenAcct;
};
